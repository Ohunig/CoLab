//
//  SearchFriendsListController.swift
//  CoLab
//
...
final class SearchFriendsListController: UIViewController {
    ...
    private struct Constants {
        ...
        static let emptyStateText = "Пользователи не найдены"
        ...
        static let searchFieldPlaceholder = "Поиск людей"
    ...
    private let interactor: SearchFriendsListBusinessLogic
    private let tableDataProvider: SearchFriendsListTableDataLogic
    ...
    private var hasLoadedUsersState = false
    ...
    init(
        interactor: SearchFriendsListBusinessLogic,
        tableDataProvider: SearchFriendsListTableDataLogic
    ) {
    ...
    private func syncUsersStateFromProvider() {
        let nextUserIds = tableDataProvider.userIds()
        let shouldReload = displayedUserIds != nextUserIds
        displayedUserIds = nextUserIds
        
        if displayedUserIds.isEmpty {
            emptyStateLabel.isHidden = !hasLoadedUsersState || isInitialLoadingShown
        } else {
            hideInitialLoading()
            emptyStateLabel.isHidden = true
        }
    ...
    private func applyUsersState(userIds: [String], animated _: Bool) {
        let shouldReload = displayedUserIds != userIds
        displayedUserIds = userIds
        hideInitialLoading()
        emptyStateLabel.isHidden = !hasLoadedUsersState || !userIds.isEmpty
    ...
    private func showInitialLoadingIfNeeded() {
        ...
        guard !hasLoadedUsersState else { return }
        guard displayedUserIds.isEmpty else { return }
    ...
    private func configure(
        ...
        with item: Model.UsersList.ViewModel.UserCell
    ) {
...
extension SearchFriendsListController: SearchFriendsListDisplayLogic {
    ...
    func displayUsers(_ viewModel: Model.UsersList.ViewModel) {
        hasLoadedUsersState = true
        hideInitialLoading()
        guard view.window != nil else { return }
        
        applyUsersState(
            userIds: viewModel.items.map { $0.id },
            animated: true
        )
    }
...
extension SearchFriendsListController: UITableViewDataSource {
    func tableView(
